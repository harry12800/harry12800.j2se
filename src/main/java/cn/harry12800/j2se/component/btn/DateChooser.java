	...
	private void initPanel() {
		...
		this.addMouseListener(new MouseAdapter() {
			...
			public void mouseExited(MouseEvent me) {
				...
				repaint();
				if (DateChooser.this.isEnabled()) {
					DateChooser.this.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
				}
			}
	...
	private class JP3 extends JPanel {
		...
		public void updateDate() {
			...
			for (MyLabel my : lm.getLabels()) {
				this.add(new MyLabelContainer(my));
			}
			select.setTime(temp);
		}
	}
...
//					jl.setForeground(Color.BLACK);
	...
	public Lunar getLunar() {
		this.selectLunar = new Lunar(this.select);
		return this.selectLunar;
	}
}