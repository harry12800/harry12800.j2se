...
public class CurrentPlayItem implements Runnable {
	...
	private void changeBackground() {
		...
		if(v>2){
			if (t > 1&&v>2)
			{
				r = (r + 1 + 256) % 256; if(r==0) r =254;
			}
			else {
				r = (r - 1 + 256) % 256; if(r==255) r =1;
			}
		}else if(v>1){
			if (t > 1)
				{g = (g + 1 + 256) % 256;; if(g==0) g=254;}
			else {
				g = (g - 1 + 256) % 256;if(g==255) g =1;
			}
		}else {
			if (t > 1)
			{	b = (b + 257) % 256;if(b==0)b =254;}
			else {
				b = (b - 1 + 256) % 256;if(b==255) b =1;
			}
		}
